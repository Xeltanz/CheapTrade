...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    public void resolveAuction(Item item) {
        ...
        if (topBid.isPresent()) {
            //aukció győztesének
            notificationService.sendEmail(topBid.get().getUser().getEmail(), "ebéj - Nyertes Aukció Lezárult", "Megnyertél egy aukciót: " + item.getName() + "\n" + CheapTradeUtils.formatBigDecimal(topBid.get().getAmount()) + " HUF" + "\nTulajdonos címe: " + item.getUser().getEmail());
            //aukció tulajdonosának
            notificationService.sendEmail(item.getUser().getEmail(), "ebéj - Aukciód Lezárult", topBid.get().getUser().getName() + " nyerte a/az " + item.getName() + " aukciódat.\n" + CheapTradeUtils.formatBigDecimal(topBid.get().getAmount()) + " HUF" + "\ncíme: " + topBid.get().getUser().getEmail());
        } else {
